...
class DatabaseManager:
    ...
    def insert_company(self, company_name, contact_person, contact_email):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    insert_query, (company_id, company_name, contact_person, contact_email)
                )
                connection.commit()
    ...
    def insert_factory(self, company_id, factory_name, factory_country, factory_city):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    insert_query,
                    (factory_id, company_id, factory_name, factory_country, factory_city),
                )
                connection.commit()
    ...
    def insert_device_type(self, device_type):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute(insert_query, (device_type_id, device_type))
                connection.commit()
    ...
    def insert_image(
        ...
        try:
            with connection.cursor() as cursor:
                insert_query = """
                INSERT INTO images (image_id, installation_id, image_url, capture_timestamp, is_trainable, image_resolution, augmentation)
    ...
    ### Insert into labels
    def insert_labels(self, label_name):
        connection = self.connect()
        if connection is None:
            return
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute(insert_query, (label_id, label_name))
                connection.commit()
    ...
    def get_installation_id(self, factory_name, device_id):
        connection = self.connect()
        ...
        query = """
            ...
            JOIN factories f ON ins.factory_id = f.factory_id
            WHERE f.factory_name = %s AND ins.device_id = %s;
        """

        try:
            with connection.cursor() as cursor:
                cursor.execute(query, (factory_name, device_id))
                result = cursor.fetchone()
                if result:
                    installation_id = result[0]
        except Exception as e:
            print(f"Failed to retrieve installation_id: {e}")
        finally:
            connection.close()
